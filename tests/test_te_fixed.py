...
def transfer_entropy_simple(x, y, lag=1, n_bins=10):
    ...
    print(f"  p_y_past_y_future non-zero entries: {np.sum(p_y_past_y_future > 0)}")
    ...
    print(f"  p_x_y_past_y_future non-zero entries: {np.sum(p_x_y_past_y_future > 0)}")
...
te = transfer_entropy_simple(x_norm[:n-1], y_norm, lag=1, n_bins=8)
...
te_ind = transfer_entropy_simple(x_norm[:n-1], z_norm, lag=1, n_bins=8)
print(f"Final TE: {te_ind:.6f}")
